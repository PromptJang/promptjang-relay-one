...
| `PJ_ONE_CLIENT` | Optional client identity written by guided MCP setup for activity diagnostics |
...
`PJ_ONE_URL`, `PJ_ONE_API_KEY`, and `PJ_ONE_CLIENT` are normally written by **Integrations → Install MCP**. There is no default-mailbox setting. Each tool call names its mailbox explicitly.
