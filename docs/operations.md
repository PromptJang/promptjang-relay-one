...
Quit the Relay One desktop app before copying its data directory. Back up both `relay-one.db` and `master.key`; API keys cannot be decrypted without the matching key.
