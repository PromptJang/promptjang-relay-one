...
export function useRelayData(request:Request) {
  ...
  async function run<T>(operation:()=>Promise<T>):Promise<T|undefined>{ error.value=''; try{return await operation()}catch(cause){error.value=cause instanceof Error?cause.message:'Request failed'} }
